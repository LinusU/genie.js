...
function DAT (raw) {
  ...
  var inflated = pako.inflateRaw(raw);
  ...
  var sound = new Array(view.getUint16());

  for (i=0; i<sound.length; i++) {

    sound[i] = {};
    sound[i].id = view.getInt32();
    sound[i].items = new Array(view.getUint16());

    view.getInt32(); // unknown

    for (j=0; j<sound[i].items.length; j++) {
      sound[i].items[j] = {};
      sound[i].items[j].filename = view.getString(13).replace(/\0+$/, '');
      sound[i].items[j].resourceId = view.getInt32();
      sound[i].items[j].probability = view.getInt16();
    }
