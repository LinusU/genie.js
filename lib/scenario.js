...
function Scenario (raw) {
  ...
  var rawLength = (raw.byteLength || raw.length);
  ...
  var compressedLength = rawLength - (headerLength + 8);
...
// view2.getUint8();
  ...
  readSeparator(view2);
  view2.getBytes(40); // victory stuff
  ...
  view2.getBytes(11520); // Unused, always 0. Yes, that's 11520 bytes.
  readSeparator(view2);
  view2.getBytes(4 * 16); // Allied vict, per-player. Ignored

  // FIXME
  view2.getBytes(20 * 16 * 4);
...
if (IS_BROWSER) {
  Scenario.load = function (path, cb) {
    ...
    req.onload = function () {
      cb(null, new Scenario(this.response));
    };
